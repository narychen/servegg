...
//    curl_easy_setopt(curl, CURLOPT_POST, 1);
    ...
    if (CURLE_OK != res) {
        loge("curl_easy_perform failed, res=%d", res);
        return "";
    }
    ...
    if (!reader.parse(strResp, value)) {
        loge("json parse failed: %s", strResp.c_str());
        return "";
    }
    
    if (value["error_code"].isNull()) {
        loge("no code in response %s", strResp.c_str());
        return "";
    }
    uint32_t nRet = value["error_code"].asUInt();
    if(nRet != 0)
    {
        loge("upload faile:%u", nRet);
        return "";
    }
...
bool CHttpClient::DownloadByteFile(const string &url, AudioMsgInfo* pAudioMsg)
{
    ...
    if(CURLE_OK != res || retcode != 200) {
        loge("curl_easy_perform failed, res=%d, ret=%u", res, retcode);
    }
    double nLen = 0;
