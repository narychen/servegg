...
void CEpollIOLoop::Run()
{
	...
	SOCKET_IO_DEBUG("add waker event sock=%d", (int)m_waker.GetWakeSocket());
	...
	while (TRUE)
	{
		...
		for (int32_t i = 0; i < nfds; i++)
		{
			...
			{
                SOCKET_IO_DEBUG("socket read event sock=%d", (int)sock);
				...
				if (pIOStream != NULL)
				{
					...
					{
						pIOStream->OnRecv();
                        SOCKET_IO_TRACE("socket recv data, sock id: %d.", (int)sock);
					}
				}
				else
				{
			...
			{
                SOCKET_IO_DEBUG("socket write event socket=%d", (int)sock);
				CBaseIOStream* pIOStream = _GetHandlerBySock(sock);
				if (pIOStream != NULL)
				{
			...
			{
                SOCKET_IO_DEBUG("socket error event sock=%d", (int)sock);
				CBaseIOStream* pIOStream = _GetHandlerBySock(sock);
                if (pIOStream != NULL)
                {
                    if (pIOStream->GetSockType() == SOCK_TCP_CLIENT && pIOStream->CheckConnect())
...
void CEpollIOLoop::Add_Handler( CBaseIOStream* piostream )
{
	...
	SOCKET_IO_DEBUG("add read event sock=%d", (int)piostream->GetSocket());
	m_waker.Wake();
	m_MapMutex.Unlock();
}
...
void CEpollIOLoop::Add_WriteEvent( CBaseIOStream* piostream )
{
	...
	if (m_MapIOStreamBySocket.find(piostream->GetSocket()) != m_MapIOStreamBySocket.end())
	{
		...
		if (piostream->GetSockType() == SOCK_TCP_CLIENT && piostream->CheckConnect())
		{
            SOCKET_IO_DEBUG("add write event for check connect sock=%d", (int)piostream->GetSocket());
			//用于判断是否connect成功
		...
		{
            SOCKET_IO_DEBUG("add write event sock=%d", (int)piostream->GetSocket());
			//可写事件
