...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			logi("select failed, error code: %d", GetLastError());
			Sleep(MIN_TIMER_DURATION);
		...
		{
			//logi("select return read count=%d\n", read_set.fd_count);
		...
		{
			//logi("select return write count=%d\n", write_set.fd_count);
		...
		{
			//logi("select return exception count=%d\n", excep_set.fd_count);
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
			...
			{
				//logi("OnRead, socket=%d\n", ev_fd);
				pSocket->OnRead();
			}

			if (events[i].filter == EVFILT_WRITE)
			{
				//logi("OnWrite, socket=%d\n", ev_fd);
				pSocket->OnWrite();
			}
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		logi("epoll_ctl() failed, errno=%d", errno);
	}
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
            ...
            {
                //logi("On Peer Close, socket=%d, ev_fd);
                pSocket->OnClose();
            }
            #endif
            // Commit End
