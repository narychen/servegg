...
CBaseSocket::~CBaseSocket()
{
	//logi("CBaseSocket::~CBaseSocket, socket=%d\n", m_socket);
}
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	logi("CBaseSocket::Listen on %s:%d", server_ip, port);
...
{
	logi("CBaseSocket::Connect, server_ip=%s, port=%d", server_ip, port);
...
#endif
			ret = 0;
			//logi("socket send block fd=%d", m_socket);
		}
		else
		{
			logi("!!!send failed, error code: %d", err_code);
		}
	}
...
void CBaseSocket::SetSendBufSize(uint32_t send_size)
{
	...
	if (ret == SOCKET_ERROR) {
		logi("set SO_SNDBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, &size, &len);
	logi("socket=%d send_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size)
{
	...
	if (ret == SOCKET_ERROR) {
		logi("set SO_RCVBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, &size, &len);
	logi("socket=%d recv_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		logi("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		logi("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		logi("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr)
{
	...
	if (pAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		if (host == NULL)
		{
			logi("gethostbyname failed, ip=%s", ip);
			return;
		}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		//logi("AcceptNewSocket, socket=%d from %s:%d\n", fd, ip_str, port);
